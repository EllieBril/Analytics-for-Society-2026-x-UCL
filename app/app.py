...
def load_data():
    base = 'data/'
    df_gap    = pd.read_csv(base + 'equity_gap_by_country_year.csv')
    df_traj   = pd.read_csv(base + 'country_trajectories.csv')
    df_scores = pd.read_csv(base + 'school_risk_scores.csv')
    df_int    = pd.read_csv(base + 'intervention_library.csv')
...
with tab3:
    ...
    @st.cache_data
    def load_shap():
        df = pd.read_csv('data/shap_importance_v6.csv')
        sc_labels = {
    ...
    <div class="insight-box">
        ...
        Our team built an XGBoost model (v6) trained on <b>613,744 students across
        80 countries</b> from the full PISA 2022 dataset, using 362 school questionnaire
        variables. SHAP analysis identified which school practices most strongly reduce
        the socioeconomic performance gap. <b>13 of these independently confirmed our
    ...
    with st.expander("📋 Model methodology and limitations"):
        st.markdown("""
        **EquiTrack v6 — XGBoost cluster model**
        ...
        **What it does:** Predicts the ESCS slope (PISA points gained per unit of
        ...
        using k-means on 362 school questionnaire variables.
        ...
        **Performance:**
        - Hold-out R² = 0.671 (n=77 test clusters)
        - 5-fold CV R² = 0.535 ± 0.068
        - LOCO CV R² = -0.923 ± 2.836 ⚠️

        **Important limitation:** The LOCO (Leave-One-Country-Out) cross-validation —
        the most honest test of generalisability — shows the model cannot reliably
        predict equity gaps in new countries. This is why EquiTrack uses the EEF
        evidence base for intervention recommendations rather than the XGBoost
        predictions directly. The SHAP feature importance is used only for
        cross-validation of the EEF findings, not for prediction.
    ...
    <div class="footer-note">
        ...
        <b>PISA Model:</b> XGBoost v6 trained on full PISA 2022 microdata
